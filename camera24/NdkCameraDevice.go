...
extern void cgoSessionStateClosed(void*, ACameraCaptureSession*);
extern void cgoSessionStateReady(void*, ACameraCaptureSession*);
extern void cgoSessionStateActive(void*, ACameraCaptureSession*);
*/
import "C"
 ...
 */
type Device C.ACameraDevice

func (device *Device) cptr() *C.ACameraDevice {
...
const (
	 ...
	 */
	ERROR_CAMERA_IN_USE = C.ERROR_CAMERA_IN_USE
	 ...
	 */
	ERROR_MAX_CAMERAS_IN_USE = C.ERROR_MAX_CAMERAS_IN_USE

	/**
	 * The camera is disabled due to a device policy, and cannot be opened.
	 */
	ERROR_CAMERA_DISABLED = C.ERROR_CAMERA_DISABLED

	/**
	 * The camera device has encountered a fatal error.
	 * <p>The camera device needs to be re-opened to be used again.</p>
	 */
	ERROR_CAMERA_DEVICE = C.ERROR_CAMERA_DEVICE
	 ...
	 */
	ERROR_CAMERA_SERVICE = C.ERROR_CAMERA_SERVICE
)
...
func (device *Device) Close() error {
	return Status(C.ACameraDevice_close(device.cptr()))
}
...
func (device *Device) GetId() string {
	return C.GoString(C.ACameraDevice_getId(device.cptr()))
}

type DeviceRequestTemplate int
...
const (
	 ...
	 */
	TEMPLATE_PREVIEW DeviceRequestTemplate = C.TEMPLATE_PREVIEW
	 ...
	 * @see ACameraDevice_createCaptureRequest
	 */
	TEMPLATE_STILL_CAPTURE DeviceRequestTemplate = C.TEMPLATE_STILL_CAPTURE
	 ...
	 * @see ACameraDevice_createCaptureRequest
	 */
	TEMPLATE_RECORD DeviceRequestTemplate = C.TEMPLATE_RECORD
	 ...
	 * @see ACameraDevice_createCaptureRequest
	 */
	TEMPLATE_VIDEO_SNAPSHOT DeviceRequestTemplate = C.TEMPLATE_VIDEO_SNAPSHOT
	 ...
	 * @see ACameraDevice_createCaptureRequest
	 */
	TEMPLATE_ZERO_SHUTTER_LAG DeviceRequestTemplate = C.TEMPLATE_ZERO_SHUTTER_LAG
	 ...
	 * @see ACameraDevice_createCaptureRequest
	 */
	TEMPLATE_MANUAL DeviceRequestTemplate = C.TEMPLATE_MANUAL
)
...
func (device *Device) CreateCaptureRequest(templateId DeviceRequestTemplate) (*CaptureRequest, error) {
	var request *C.ACaptureRequest
	ret := Status(C.ACameraDevice_createCaptureRequest(device.cptr(), C.ACameraDevice_request_template(templateId), &request))
...
func (output *CaptureSessionOutput) cptr() *C.ACaptureSessionOutput {
	return (*C.ACaptureSessionOutput)(output)
}
...
func CaptureSessionOutputContainerCreate() (*CaptureSessionOutputContainer, error) {
	var container *C.ACaptureSessionOutputContainer
	ret := Status(C.ACaptureSessionOutputContainer_create(&container))
	return (*CaptureSessionOutputContainer)(container), ret
}
...
func (container *CaptureSessionOutputContainer) Free() {
	C.ACaptureSessionOutputContainer_free(container.cptr())
}
...
func (output *CaptureSessionOutput) Free() {
	C.ACaptureSessionOutput_free(output.cptr())
}
...
func (container *CaptureSessionOutputContainer) Add(output *CaptureSessionOutput) error {
	return Status(C.ACaptureSessionOutputContainer_add(container.cptr(), output.cptr()))
}
...
//        ACaptureSessionOutputContainer* container, const ACaptureSessionOutput* output);
func (container *CaptureSessionOutputContainer) Remove(output *CaptureSessionOutput) error {
	return Status(C.ACaptureSessionOutputContainer_remove(container.cptr(), output.cptr()))
}
...
func (device *Device) CreateCaptureSession(outputs *CaptureSessionOutputContainer, cb CaptureSessionStateCallbacks) (*CaptureSession, error) {
	var session *C.ACameraCaptureSession
	var stateCB C.ACameraCaptureSession_stateCallbacks
	stateCB.onClosed = (C.ACameraCaptureSession_stateCallback)(C.cgoSessionStateClosed)
	stateCB.onReady = (C.ACameraCaptureSession_stateCallback)(C.cgoSessionStateReady)
	stateCB.onActive = (C.ACameraCaptureSession_stateCallback)(C.cgoSessionStateActive)
	ret := Status(C.ACameraDevice_createCaptureSession(device.cptr(), outputs.cptr(), &stateCB, &session))
	if ret == nil {
		captureSessionMap[session] = cb
	}
	return (*CaptureSession)(session), ret
}

var captureSessionMap = map[*C.ACameraCaptureSession]CaptureSessionStateCallbacks{}

//export cgoSessionStateClosed
func cgoSessionStateClosed(context unsafe.Pointer, session *C.ACameraCaptureSession) {
	if o, ok := captureSessionMap[session]; ok && o != nil {
		o.OnClosed((*CaptureSession)(session))
	}
	delete(captureSessionMap, session)
}

//export cgoSessionStateReady
func cgoSessionStateReady(context unsafe.Pointer, session *C.ACameraCaptureSession) {
	if o, ok := captureSessionMap[session]; ok && o != nil {
		o.OnReady((*CaptureSession)(session))
	}
}

//export cgoSessionStateActive
func cgoSessionStateActive(context unsafe.Pointer, session *C.ACameraCaptureSession) {
	if o, ok := captureSessionMap[session]; ok && o != nil {
		o.OnActive((*CaptureSession)(session))
	}
}
